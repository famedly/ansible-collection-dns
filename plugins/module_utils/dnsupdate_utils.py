...
class ResourceRecord():
  ...
  def __eq__(self, other):
    return self.name == other.name and self.typ == other.typ and self.content == other.content
...
def get_diff_rr_set(existing_rr_set: list[ResourceRecord], rr_set: list[ResourceRecord], mode: str) -> ChangeSet:
  changeset = ChangeSet()
  if mode == 'present':
...
def get_resource_records(rr_set: list[ResourceRecord], _resolver: resolver.Resolver = default_resolver) -> list[ResourceRecord]:
  existing_rrs = []
  for rr in rr_set:
     existing_rrs.extend(get_resource_record(rr, _resolver))
  return existing_rrs

def get_resource_record(rr: ResourceRecord, _resolver: resolver.Resolver) -> list[ResourceRecord]:
  qname: str = rr.name
  qtype: str = rr.typ
  try:
